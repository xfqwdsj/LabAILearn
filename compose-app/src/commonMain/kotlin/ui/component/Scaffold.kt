...
private fun ScaffoldLayout(
    ...
    SubcomposeLayout { constraints ->
        ...
        val bottomBarPlaceables =
            subcompose(ScaffoldLayoutContent.BottomBar) { bottomBar() }.fastMap { it.measure(looseConstraints) }
        ...
        val bodyContentPlaceables = subcompose(ScaffoldLayoutContent.MainContent) {
            ...
            val topBarHeight = topBarHeight.toDp()
            val bottomBarHeight = bottomBarHeight?.toDp() ?: 0.dp

            val innerPadding = PaddingValues(
                top = max(insetsTop, topBarHeight),
                bottom = max(insetsBottom, bottomBarHeight),
...
private enum class ScaffoldLayoutContent {
    TopBar, MainContent, Snackbar, Fab, BottomBar
}
